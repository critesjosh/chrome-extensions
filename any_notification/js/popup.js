...
document.getElementById('set').addEventListener('click', function(){
  ...
  if (time < 1) {
    ...
    chrome.storage.local.get(function(storage){
      notifactionsList = storage.notifactionsList;
      var data = {timeLength: time, title: title, text: text};
      notificationsList.push(data);
    });

    //TODO: add this notifcation to a list of notifactions that the user has saved

    chrome.storage.local.set({notificationsList: notificationsList}, function() {
